var React;

var FacebookWidget= React.createClass({
  displayName: 'facebook-widget',
  url        : 'http://www.agendakar.com/api/events.json',

  getInitialState: function(){
    return {
      graph_url : 'https://graph.facebook.com/',
      pseudo    : 'hkairi',
      taille    : 200,
      options   : 'picture?width=200',
      image_url : ''
    };
  },

  set_image_url: function( pseudo ){
    var _url = this.state.graph_url + pseudo + '/' + this.state.options;
    this.setState({ image_url : _url });
  },

  clickHandler: function(){
    var _pseudo = this.refs.f_id.getDOMNode().value ;
    this.set_image_url( _pseudo );
  },

  render: function(){
    return(
      React.createElement("div", null, 
        React.createElement("img", {src: this.state.image_url}), 
        React.createElement("input", {type: "text", ref: "f_id", placeholder: "Votre ID Facebook", onChange: this.clickHandler}), 
        React.createElement("input", {type: "submit", onClick: this.clickHandler})
      )
    );
  }
});

React.render(
  React.createElement(FacebookWidget, null), document.getElementById('content')
);
